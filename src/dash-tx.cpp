...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " datos-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  datos-tx [options] <hex-tx> [commands]  Update hex-encoded datos transaction\n" +
            "or:     datos-tx [options] -create [commands]   Create hex-encoded datos transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded datos transaction
