...
static bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/datos.conf are parsed in qt/datos.cpp's main()
    SetupServerArgs();
    std::string error;
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  datosd [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see datosd -h for a list of options.\n", argv[i])));
            }
        }

        // -server defaults to true for datosd but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect datosd signal handlers
    noui_connect();
