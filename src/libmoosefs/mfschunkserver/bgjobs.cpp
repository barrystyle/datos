    ...
    for (;;) {
        ...
        case OP_GETBLOCKS:
            if (jstate == JSTATE_DISABLED) {
                ...
                status = hdd_get_blocks(ijargs->chunkid, ijargs->version, (uint8_t*)ijargs->pointer);
            }
            break;
        case OP_GETCHECKSUM:
            if (jstate == JSTATE_DISABLED) {
                ...
                status = hdd_get_checksum(ijargs->chunkid, ijargs->version, (uint8_t*)ijargs->pointer);
            }
            break;
        case OP_GETCHECKSUMTAB:
            if (jstate == JSTATE_DISABLED) {
                ...
                status = hdd_get_checksum_tab(ijargs->chunkid, ijargs->version, (uint8_t*)ijargs->pointer);
            }
            break;
        case OP_CHUNKMOVE:
            if (jstate == JSTATE_DISABLED) {
...
uint32_t job_serv_read(void (*callback)(uint8_t status, void* extra), void* extra, int sock, const uint8_t* packet, uint32_t length)
{
    jobpool* jp = globalpool;
    chunk_rw_args* args;
    args = (chunk_rw_args*)malloc(sizeof(chunk_rw_args));
    passert(args);
    args->sock = sock;
    args->packet = packet;
    args->length = length;
...
uint32_t job_get_chunk_blocks(void (*callback)(uint8_t status, void* extra), void* extra, uint64_t chunkid, uint32_t version, uint8_t* blocks)
{
    jobpool* jp = globalpool;
    chunk_ij_args* args;
    args = (chunk_ij_args*)malloc(sizeof(chunk_ij_args));
    passert(args);
    args->chunkid = chunkid;
    args->version = version;
    args->pointer = blocks;
...
uint32_t job_chunk_move(void (*callback)(uint8_t status, void* extra), void* extra, void* fsrc, void* fdst)
{
    jobpool* jp = globalpool;
    chunk_mv_args* args;
    args = (chunk_mv_args*)malloc(sizeof(chunk_mv_args));
    passert(args);
    args->fsrc = fsrc;
    args->fdst = fdst;
...
int job_init(void)
{
    globalpool = (jobpool*)job_pool_new();
    if (globalpool == NULL) {
        return -1;
    }
    job_reload();
