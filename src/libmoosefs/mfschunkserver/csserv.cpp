...
void csserv_get_version(csserventry* eptr, const uint8_t* data, uint32_t length)
{
    ...
    put16bit(&ptr, VERSION_MAJ);
    put8bit(&ptr, VERSION_MID);
    put8bit(&ptr, VERSION_MIN);
...
void csserv_get_chunk_checksum_tab(csserventry* eptr, const uint8_t* data, uint32_t length)
{
    ...
    ij = (idlejob*)malloc(MYSIZE(idlejob, buff, 4096));
...
void csserv_reload(void)
{
    ...
    newListenHost = strdup(localnode.listenhost);
    newListenPort = strdup(localnode.listenport);
...
int csserv_init(void)
{
    ...
    ListenHost = strdup(localnode.listenhost);
    ListenPort = strdup(localnode.listenport);
