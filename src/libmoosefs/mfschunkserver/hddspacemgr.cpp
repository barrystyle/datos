...
// set for target disk gb
class TargetDisk;
TargetDisk chunkservconf;
...
void hdd_report_changed_chunk(uint64_t chunkid, uint32_t version)
{
    ...
    if (chgchunks && chgchunks->chunksinblock < CHGCHUNKSBLOCKSIZE) {
        ...
        xc = (chgchunk*)malloc(sizeof(newchunk));
        passert(xc);
...
static inline void hdd_add_chunk_to_folder(chunk* c, folder* f)
{
    if (f->chunkcount == f->chunktabsize) {
        if (f->chunktabsize == 0) {
            ...
            f->chunktab = (chunk**)malloc(sizeof(chunk*) * f->chunktabsize);
        } else {
            f->chunktabsize *= 3;
            f->chunktabsize /= 2;
            f->chunktab = (chunk**)realloc(f->chunktab, sizeof(chunk*) * f->chunktabsize);
        }
        passert(f->chunktab);
    }
...
static inline void hdd_folder_dump_chunkdb_end(folder* f)
{
    if (f->dumpfd >= 0) {
        ...
        pleng = strlen(f->path);
        fname_src = (char*)malloc(pleng + 13);
        fname_dst = (char*)malloc(pleng + 9);
        passert(fname_src);
        passert(fname_dst);
...
void hdd_check_folders(void)
{
    ...
    // effectively 'business as usual'.. dont spam logs
...
#endif
            // this will always spam the logs, better to disable
...
chunk* hdd_random_chunk(folder* f)
{
    ...
    zassert(pthread_mutex_lock(&hashlock));
    if (f->chunkcount > 0) {
        for (otry = 0; otry < RANDOM_CHUNK_RETRIES; otry++) {
...
static int hdd_int_move(folder* fsrc, folder* fdst)
{
    ...
    if (!(
            (fsrc->damaged == 0 && fsrc->toremove == REMOVING_NO && fsrc->markforremoval == MFR_NO && fsrc->scanstate == SCST_WORKING && fsrc->total > 0) && (fdst->damaged == 0 && fdst->toremove == REMOVING_NO && fdst->markforremoval == MFR_NO && fdst->scanstate == SCST_WORKING && fdst->total > 0 && fdst->wfrcount == 0))) {
        zassert(pthread_mutex_unlock(&folderlock));
        return MFS_ERROR_NOTDONE;
    }
    zassert(pthread_mutex_unlock(&folderlock));
    ...
    tmp_filename = (char*)malloc(leng + 13 + 16 + 1);
...
void* hdd_highspeed_rebalance_thread(void* arg)
{
    ...
    for (;;) {
        ...
        if (sstatus == 1) { // have servers
            ...
            r = (rebalance*)malloc(sizeof(rebalance));
            passert(r);
            r->fsrc = fsrc;
            r->fdst = fdst;
...
static inline int hdd_folder_fastscan(folder* f, char* fullname, uint16_t plen)
{
    ...
    //		return -1;
    //	}
    chunkbuff = (uint8_t*)malloc(sb.st_size);
    if (chunkbuff == NULL) {
        close(fd);
        return -1;
    }
...
{ // metaid has been verified with master - write .metaid files
    folder* f;
    char* metaidfname;
    int mfd;
    uint32_t l;
...
static inline double sizestrtod(const char* str, const char** endptr)
{
    double val, frac;
    int f;
    val = 0.0;
...
int hdd_parseline(char* hddcfgline)
{
    ...
    if (cannotbeused) {
        mfs_arg_syslog(LOG_WARNING, "hdd space manager: drive '%s' is being removed and can not be added again while removing is in progress - otry it again in couple of seconds", pptr);
        return -1;
    }
    ...
    // add line to cfgline
    cl = (cfgline*)malloc(sizeof(cfgline));
    passert(cl);
    cl->path = (char*)malloc(l + 1);
    passert(cl->path);
...
int hdd_folders_reinit(void)
{
    ...
    // default to 25GiB if not set at startup
    sprintf(buff, "%s %dGiB", localnode.chunkpath, chunkservconf.get_target_space());
    hdd_parseline(buff);
...
static inline void hdd_options_common(uint8_t initflag)
{
    ...
    zassert(pthread_mutex_lock(&folderlock));
    HDDErrorCount = HDD_ERROR_TOLERANCE_COUNT;
    HDDErrorTime = HDD_ERROR_TOLERANCE_PERIOD;
    HDDRoundRobinChunkCount = HDD_RR_CHUNK_COUNT;
    tmp = HDD_KEEP_DUPLICATES_HOURS;
    ...
    zassert(pthread_mutex_unlock(&folderlock));
    zassert(pthread_mutex_lock(&testlock));
    HDDTestMBPS = HDD_TEST_SPEED;
    HDDRebalancePerc = HDD_REBALANCE_UTILIZATION;
    HSRebalanceLimit = HDD_HIGH_SPEED_REBALANCE_LIMIT;
    MinTimeBetweenTests = HDD_MIN_TEST_INTERVAL;
    MinFlushCacheTime = HDD_FADVISE_MIN_TIME;
    zassert(pthread_mutex_unlock(&testlock));
    zassert(pthread_mutex_lock(&doplock));
    DoFsyncBeforeClose = HDD_FSYNC_BEFORE_CLOSE;
    zassert(pthread_mutex_unlock(&doplock));
    LeaveFree = HDD_LEAVE_SPACE_DEFAULT;
    sp = HDD_SPARSIFY_ON_WRITE;
#ifdef HAVE___SYNC_OP_AND_FETCH
    if (sp) {
