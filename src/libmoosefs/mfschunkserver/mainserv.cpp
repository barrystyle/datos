...
uint8_t mainserv_write_middle(int sock, int fwdsock, uint64_t gchunkid, uint32_t gversion)
{
    ...
    pdataleng = 4096;
#ifdef MMAP_ALLOC
    pdata = (uint8_t*)mmap(NULL, pdataleng, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
#else
...
#ifdef MMAP_ALLOC
                    wrjob = (write_job*)mmap(NULL, offsetof(write_job, data) + leng + 8, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
#else
...
#ifdef MMAP_ALLOC
                    wrjob = (write_job*)mmap(NULL, offsetof(write_job, data), PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
#else
