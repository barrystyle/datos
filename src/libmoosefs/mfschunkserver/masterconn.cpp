...
enum { IJ_GET_CHUNK_BLOCKS,
    IJ_GET_CHUNK_CHECKSUM,
    IJ_GET_CHUNK_CHECKSUM_TAB };
...
void* masterconn_create_detached_packet(masterconn* eptr, uint32_t type, uint32_t size)
{
    ...
    psize = size + 8;
    outpacket = (out_packetstruct*)malloc(offsetof(out_packetstruct, data) + psize);
...
void masterconn_reportload(void)
{
    ...
    uint8_t rebalance;
    uint8_t* buff;
    if (eptr->mode == DATA && eptr->registerstate == REGISTERED) {
...
void masterconn_get_chunk_blocks(masterconn* eptr, const uint8_t* data, uint32_t length)
{
    ...
    ij = (idlejob*)malloc(MYSIZE(idlejob, buff, 2));
...
void masterconn_get_chunk_checksum(masterconn* eptr, const uint8_t* data, uint32_t length)
{
    ...
    ij = (idlejob*)malloc(MYSIZE(idlejob, buff, 4));
...
void masterconn_get_chunk_checksum_tab(masterconn* eptr, const uint8_t* data, uint32_t length)
{
    ...
    ij = (idlejob*)malloc(MYSIZE(idlejob, buff, 4 * MFSBLOCKSINCHUNK));
...
void masterconn_read(masterconn* eptr, double now)
{
    ...
    if (readbuffsize == 0) {
        readbuffsize = 65536;
        readbuff = (uint8_t*)malloc(readbuffsize);
        passert(readbuff);
    }
    ...
    for (;;) {
        ...
        if (i == 0) {
            ...
            if (rbleng == readbuffsize) {
                readbuffsize *= 2;
                readbuff = (uint8_t*)mfsrealloc(readbuff, readbuffsize);
                passert(readbuff);
            } else {
                break;
            }
    ...
    while (rbpos < rbleng) {
        ...
        if (eptr->input_packet == NULL) {
            ...
            eptr->input_packet = (in_packetstruct*)malloc(offsetof(in_packetstruct, data) + leng);
...
int masterconn_init(void)
{
    ...
    ChunksPerRegisterPacket = CHUNKS_PER_REGISTER_PACKET;
