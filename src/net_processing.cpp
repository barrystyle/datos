 ...
 *  We have 4 times smaller block times in datos, so we need to push 4 times more invs per 1MB. */
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        datos Related Inventory Messages
...
bool static ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (msg_type == NetMsgType::NETPROOF) {
        ...
        if (!netproof.Check()) {
            Misbehaving(pfrom->GetId(), 0, strprintf("empty netproof. peer=%d", pfrom->GetId()));
            return false;
        }
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // DASH this code also handles non-TXs (datos specific messages)
