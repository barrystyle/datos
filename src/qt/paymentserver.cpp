...
const QString BITCOIN_IPC_PREFIX("pac:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/pacprotocol-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/pacprotocol-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/pacprotocol-paymentrequest";
#endif
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("pac://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'pac://' is not a valid URI. Use 'pac:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // pac: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid pacprotocol address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom pacprotocol addresses are not supported
        ...
        // pacprotocol amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
