...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // users ignore all instructions on pacprotocolcentral etc. and do not save them...
...
static void gobject_vote_conf_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"gobject vote-conf",
        "Vote on a governance object by masternode configured in pacprotocol.conf\n",
        {
...
static UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("pacprotocol.conf", statusObj);
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "pacprotocol",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "pacprotocol",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "pacprotocol",               "gobject",                &gobject,                {} },
    { "pacprotocol",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
