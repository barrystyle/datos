...
BOOST_FIXTURE_TEST_CASE(dip3_test_mempool_reorg, TestChainDIP3Setup)
{
    ...
    FundTransaction(tx_collateral, utxos, scriptCollateral, 500000 * COIN, coinbaseKey);
    ...
    for (size_t i = 0; i < tx_collateral.vout.size(); ++i) {
        if (tx_collateral.vout[i].nValue == 500000 * COIN) {
    ...
    FundTransaction(tx_reg, utxos, scriptPayout, 500000 * COIN, coinbaseKey);
...
BOOST_FIXTURE_TEST_CASE(dip3_test_mempool_dual_proregtx, TestChainDIP3Setup)
{
    ...
    for (size_t i = 0; i < tx_reg1.vout.size(); ++i) {
        if (tx_reg1.vout[i].nValue == 500000 * COIN) {
    ...
    FundTransaction(tx_reg2, utxos, scriptPayout, 500000 * COIN, coinbaseKey);
