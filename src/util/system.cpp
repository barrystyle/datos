...
const char * const BITCOIN_CONF_FILENAME = "datos.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "datosd -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //datos-tx using stdin
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Datos
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Datos
    // Mac: ~/Library/Application Support/Datos
    // Unix: ~/.datos
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Datos";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Datos";
#else
    // Unix
    return pathRet / ".datos";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty datos.conf if it does not exist
...
//    std::string strCopyrightHolders = strPrefix + strprintf(" %u-%u ", nStartYear, nEndYear) + strprintf(_(COPYRIGHT_HOLDERS).translated, _(COPYRIGHT_HOLDERS_SUBSTITUTION));
    ...
    // Check for untranslated substitution to make sure datos copyright is not removed by accident
    if (copyright_devs.find("datos") == std::string::npos) {
